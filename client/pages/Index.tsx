...
export default function Index() {
  ...
  const [form, setForm] = useState({ username: "", email: "", password: "", role: "user" as Role, departmentId: "" });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        await register(form.username, form.email, form.role, form.role === "staff" ? form.departmentId || undefined : undefined);
      }
      nav("/dashboard");
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-secondary/60 to-background">
      <div className="mx-auto grid max-w-6xl gap-10 px-6 py-16 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          <p className="mt-3 text-lg text-muted-foreground">{t("landing_intro")}</p>
          <ul className="mt-6 grid gap-3 text-sm text-foreground/80">
            <li className="flex items-start gap-2"><span className="mt-1 text-emerald-600">✓</span> Role-based access: Admin, Staff, Citizen</li>
            <li className="flex items-start gap-2"><span className="mt-1 text-emerald-600">✓</span> Dashboard KPIs, notifications, and quick actions</li>
            <li className="flex items-start gap-2"><span className="mt-1 text-emerald-600">✓</span> Issues table, per-issue chat, and exports</li>
            <li className="flex items-start gap-2"><span className="mt-1 text-emerald-600">✓</span> Map with clustering and heatmap (placeholder)</li>
            <li className="flex items-start gap-2"><span className="mt-1 text-emerald-600">✓</span> i18n (EN/HI) and Dark mode</li>
          </ul>
        </div>
        <div>
                      ...
                      {(["admin", "staff", "user"] as Role[]).map((r) => (
                        <button
                          ...
                          className={`rounded-md border px-3 py-2 text-sm ${
                            form.role === r ? "border-primary bg-secondary" : "hover:bg-muted"
                          }`}
                        >
                          {r}
                        </button>
